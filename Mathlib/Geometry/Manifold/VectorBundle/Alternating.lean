...
The proof goes through `ContinuousAlternatingMap.compContinuousLinearMapCLM_contDiff`, which
exhibits the pullback operator as the diagonal of a continuous multilinear lift constructed via
the polarization formula. The polarization requires inverting `Fintype.card ι !`, hence the
`[CharZero 𝕜]` hypothesis.
-/
...
variable {𝕜 : Type*} [NontriviallyNormedField 𝕜] [CharZero 𝕜] {ι : Type*} [Fintype ι]
...
/-- If `E₁` and `E₂` are `C^n` vector bundles over a `CharZero` field with `F₂` complete, then
the bundle of continuous `ι`-slot alternating maps from `E₁` to `E₂` is also a `C^n` vector
bundle (for any `n : WithTop ℕ∞`, including `ω`). -/
