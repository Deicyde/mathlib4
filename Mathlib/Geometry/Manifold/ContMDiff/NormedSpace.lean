...
Authors: Sébastien Gouëzel, Floris van Doorn, Jack McCarthy
-/
module
...
* `ContinuousAlternatingMap.contMDiff`: continuous alternating maps between normed spaces are smooth
...
/-! ### Alternating maps between normed spaces are smooth -/
...
theorem ContMDiffWithinAt.continuousAlternatingMap_precomp {n : ℕ∞}
    {f : M → F₁ →L[𝕜] F₂}
    (hf : ContMDiffWithinAt I 𝓘(𝕜, F₁ →L[𝕜] F₂) (n : WithTop ℕ∞) f s x) :
    ContMDiffWithinAt I 𝓘(𝕜, (F₂ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₁ [⋀^ι]→L[𝕜] F₃)) (n : WithTop ℕ∞)
      (fun y ↦ compContinuousLinearMapCLM (F := F₃) (ι := ι) (f y)) s x :=
  ContDiff.comp_contMDiffWithinAt (compContinuousLinearMapCLM_contDiff) hf

nonrec theorem ContMDiffAt.continuousAlternatingMap_precomp {n : ℕ∞}
    {f : M → F₁ →L[𝕜] F₂}
    (hf : ContMDiffAt I 𝓘(𝕜, F₁ →L[𝕜] F₂) (n : WithTop ℕ∞) f x) :
    ContMDiffAt I 𝓘(𝕜, (F₂ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₁ [⋀^ι]→L[𝕜] F₃)) (n : WithTop ℕ∞)
      (fun y ↦ compContinuousLinearMapCLM (F := F₃) (ι := ι) (f y)) x :=
  hf.continuousAlternatingMap_precomp

theorem ContMDiffOn.continuousAlternatingMap_precomp {n : ℕ∞} {f : M → F₁ →L[𝕜] F₂}
    (hf : ContMDiffOn I 𝓘(𝕜, F₁ →L[𝕜] F₂) (n : WithTop ℕ∞) f s) :
    ContMDiffOn I 𝓘(𝕜, (F₂ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₁ [⋀^ι]→L[𝕜] F₃)) (n : WithTop ℕ∞)
      (fun y ↦ compContinuousLinearMapCLM (F := F₃) (ι := ι) (f y)) s :=
  fun x hx ↦ (hf x hx).continuousAlternatingMap_precomp

theorem ContMDiff.continuousAlternatingMap_precomp {n : ℕ∞} {f : M → F₁ →L[𝕜] F₂}
    (hf : ContMDiff I 𝓘(𝕜, F₁ →L[𝕜] F₂) (n : WithTop ℕ∞) f) :
    ContMDiff I 𝓘(𝕜, (F₂ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₁ [⋀^ι]→L[𝕜] F₃)) (n : WithTop ℕ∞)
      (fun y ↦ compContinuousLinearMapCLM (F := F₃) (ι := ι) (f y)) :=
  fun x ↦ (hf x).continuousAlternatingMap_precomp

end CompleteCodomain
...
theorem ContMDiffWithinAt.continuousAlternatingMap_postcomp {f : M → F₂ →L[𝕜] F₃}
  ...
  ContDiff.comp_contMDiffWithinAt (F' := (F₁ [⋀^ι]→L[𝕜] F₂) →L[𝕜] (F₁ [⋀^ι]→L[𝕜] F₃))
    (g := ContinuousLinearMap.compContinuousAlternatingMapCLM 𝕜 F₁ F₂ F₃ ι)
    (ContinuousLinearMap.contDiff _) hf

nonrec theorem ContMDiffAt.continuousAlternatingMap_postcomp {f : M → F₂ →L[𝕜] F₃}
    (hf : ContMDiffAt I 𝓘(𝕜, F₂ →L[𝕜] F₃) n f x) :
    ContMDiffAt I 𝓘(𝕜, (F₁ [⋀^ι]→L[𝕜] F₂) →L[𝕜] (F₁ [⋀^ι]→L[𝕜] F₃)) n
      (fun y ↦ ContinuousLinearMap.compContinuousAlternatingMapCLM 𝕜 F₁ F₂ F₃ ι (f y)) x :=
  hf.continuousAlternatingMap_postcomp

theorem ContMDiffOn.continuousAlternatingMap_postcomp {f : M → F₂ →L[𝕜] F₃}
    (hf : ContMDiffOn I 𝓘(𝕜, F₂ →L[𝕜] F₃) n f s) :
    ContMDiffOn I 𝓘(𝕜, (F₁ [⋀^ι]→L[𝕜] F₂) →L[𝕜] (F₁ [⋀^ι]→L[𝕜] F₃)) n
      (fun y ↦ ContinuousLinearMap.compContinuousAlternatingMapCLM 𝕜 F₁ F₂ F₃ ι (f y)) s :=
  fun x hx ↦ (hf x hx).continuousAlternatingMap_postcomp

theorem ContMDiff.continuousAlternatingMap_postcomp {f : M → F₂ →L[𝕜] F₃}
...
theorem ContMDiffWithinAt.cle_continuousAlternatingMapCongr {n : ℕ∞}
    {f : M → F₁ ≃L[𝕜] F₂} {g : M → F₃ ≃L[𝕜] F₄}
    (hf : ContMDiffWithinAt I 𝓘(𝕜, F₂ →L[𝕜] F₁) (n : WithTop ℕ∞)
      (fun x ↦ ((f x).symm : F₂ →L[𝕜] F₁)) s x)
    (hg : ContMDiffWithinAt I 𝓘(𝕜, F₃ →L[𝕜] F₄) (n : WithTop ℕ∞)
      (fun x ↦ (g x : F₃ →L[𝕜] F₄)) s x) :
    ContMDiffWithinAt I 𝓘(𝕜, (F₁ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₂ [⋀^ι]→L[𝕜] F₄)) (n : WithTop ℕ∞)
      (fun y ↦ ((f y).continuousAlternatingMapCongr (g y) (ι := ι) :
        (F₁ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₂ [⋀^ι]→L[𝕜] F₄))) s x :=
  show ContMDiffWithinAt I 𝓘(𝕜, _) (n : WithTop ℕ∞)
    (fun y ↦ (ContinuousLinearMap.compContinuousAlternatingMapCLM 𝕜 F₂ F₃ F₄ ι (g y)).comp
      (compContinuousLinearMapCLM (F := F₃) ((f y).symm : F₂ →L[𝕜] F₁))) s x
  from hg.continuousAlternatingMap_postcomp.clm_comp hf.continuousAlternatingMap_precomp

nonrec theorem ContMDiffAt.cle_continuousAlternatingMapCongr {n : ℕ∞}
    {f : M → F₁ ≃L[𝕜] F₂} {g : M → F₃ ≃L[𝕜] F₄}
    (hf : ContMDiffAt I 𝓘(𝕜, F₂ →L[𝕜] F₁) (n : WithTop ℕ∞)
      (fun x ↦ ((f x).symm : F₂ →L[𝕜] F₁)) x)
    (hg : ContMDiffAt I 𝓘(𝕜, F₃ →L[𝕜] F₄) (n : WithTop ℕ∞)
      (fun x ↦ (g x : F₃ →L[𝕜] F₄)) x) :
    ContMDiffAt I 𝓘(𝕜, (F₁ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₂ [⋀^ι]→L[𝕜] F₄)) (n : WithTop ℕ∞)
      (fun y ↦ ((f y).continuousAlternatingMapCongr (g y) (ι := ι) :
        (F₁ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₂ [⋀^ι]→L[𝕜] F₄))) x :=
  hf.cle_continuousAlternatingMapCongr hg

theorem ContMDiffOn.cle_continuousAlternatingMapCongr {n : ℕ∞}
    {f : M → F₁ ≃L[𝕜] F₂} {g : M → F₃ ≃L[𝕜] F₄}
    (hf : ContMDiffOn I 𝓘(𝕜, F₂ →L[𝕜] F₁) (n : WithTop ℕ∞)
      (fun x ↦ ((f x).symm : F₂ →L[𝕜] F₁)) s)
    (hg : ContMDiffOn I 𝓘(𝕜, F₃ →L[𝕜] F₄) (n : WithTop ℕ∞)
      (fun x ↦ (g x : F₃ →L[𝕜] F₄)) s) :
    ContMDiffOn I 𝓘(𝕜, (F₁ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₂ [⋀^ι]→L[𝕜] F₄)) (n : WithTop ℕ∞)
      (fun y ↦ ((f y).continuousAlternatingMapCongr (g y) (ι := ι) :
        (F₁ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₂ [⋀^ι]→L[𝕜] F₄))) s :=
  fun x hx ↦ (hf x hx).cle_continuousAlternatingMapCongr (hg x hx)

theorem ContMDiff.cle_continuousAlternatingMapCongr {n : ℕ∞}
    {f : M → F₁ ≃L[𝕜] F₂} {g : M → F₃ ≃L[𝕜] F₄}
    (hf : ContMDiff I 𝓘(𝕜, F₂ →L[𝕜] F₁) (n : WithTop ℕ∞)
      (fun x ↦ ((f x).symm : F₂ →L[𝕜] F₁)))
    (hg : ContMDiff I 𝓘(𝕜, F₃ →L[𝕜] F₄) (n : WithTop ℕ∞)
      (fun x ↦ (g x : F₃ →L[𝕜] F₄))) :
    ContMDiff I 𝓘(𝕜, (F₁ [⋀^ι]→L[𝕜] F₃) →L[𝕜] (F₂ [⋀^ι]→L[𝕜] F₄)) (n : WithTop ℕ∞)
      (fun y ↦ ((f y).continuousAlternatingMapCongr (g y) (ι := ι) :
